...
function CreateTest(props) {
        ...
        if (res?.status) {
          ...
          setTestObj({
            ...
            scheduledTime: scheduledTime
              ? moment(scheduledTime, "HH:mm A").format("HH:mm")
              : "",
          });
    ...
    Api("post", `question/fatchRandomeQuestions`, testObj, router).then(
      async (res) => {
        ...
        if (res?.status) {
          ...
          let questionIdArray = [];
          res.data.map((item) => {
            questionIdArray = questionIdArray.concat(
              item.que.map((q) => q._id)
            );
          });
  ...
  const updatequizz = () => {
    ...
    data.scheduledTime = moment(data.scheduledTime, "HH:mm").format("hh:mm a")
    props.loader(true);
  ...
  return (
    <div className="bg-white min-h-full py-10 px-5">
      <div className="border-2 rounded-[15px] border-[var(--custom-blue)] p-5">
        <p className="text-2xl font-bold text-black MerriweatherSans">{`${moment(
          new Date()
        ).format("DD-MMM-YYYY")} , ${moment(new Date()).format("dddd")}`}</p>
        <p className="md:text-4xl text-3xl font-bold text-black MerriweatherSans pt-2">
          Hello <span className="text-[var(--custom-blue)]">{user?.name}</span>
        </p>
      </div>

      <form
        className="border-2 border-[var(--custom-blue)] rounded-[15px] w-full mt-5 p-5 bg-[var(--dark-lightBlue)]"
        onSubmit={hanledForm}
      >
        <div className="grid md:grid-cols-2 grid-cols-1 w-full md:gap-5">
          ...
          {testObj?.scheduledDate && (
            <div>
              ...
              <input
                type="date"
                value={testObj?.scheduledDate}
                onChange={(e) =>
                  setTestObj({ ...testObj, scheduledDate: e.target.value })
                }
                min={getTodayDate()}
                className="outline-none p-2 w-full  border border-[var(--custom-blue)] rounded-[7px] bg-white"
              />
          ...
          {testObj?.scheduledTime && (
            <div>
              ...
              <input
                type="time"
                value={testObj?.scheduledTime}
                onChange={(e) =>
                  setTestObj({ ...testObj, scheduledTime: e.target.value })
                }
                className="outline-none p-2 w-full  border border-[var(--custom-blue)] rounded-[7px] bg-white"
              />
            </div>
        ...
        <div className="grid md:grid-cols-3 grid-cols-1 w-full md:gap-5 p-2 border-2 border-[var(--custom-blue)] rounded-md mt-10">
          ...
          <div className="flex items-end justify-end">
            <button
              className="bg-[var(--custom-blue)] h-[52px] w-60 rounded-[7px] text-white text-base font-normal flex justify-center items-center mt-5"
              type="button"
              onClick={getQuizzData}
            >
              Fetch Rendom Questions
            </button>
          </div>
        </div>
          ...
          <table className="w-full border-collapse mb-1">
            <thead>
              <tr>
                ...
                <th className="border border-[#ccc] p-2 bg-[#f0f0f0]">
                  Ouestion Name
                </th>
                <th className="border border-[#ccc] p-2 bg-[#f0f0f0]">
                  Category
            ...
            <tbody>
              {item?.que.map((qu, inx) => (
                <tr key={inx}>
                  ...
                  <td className="border border-[#ccc] p-2">
                    {qu.option.map((items, ins) => (
                      <p key={ins} className="text-black text-xs font-normal">
                  ...
                  <td className="border border-[#ccc] p-2">
                    <img src={qu.image} className="h-10 w-10" />
                  </td>
                </tr>
              ))}
            </tbody>
