...
function Categories(props) {
  ...
  const getallproducttype = async () => {
    //**************** */
    props.loader(true);
    Api("get", "category", "", router).then(
      (res) => {
        props.loader(false);
        console.log("res================> form data :: ", res);
        setloadTypeData(res.data.filter(f => f._id));
      },
      (err) => {
        props.loader(false);
        console.log(err);
  ...
  const submit = (e) => {
    ...
    createUpdateApi(method, url, data)
  };
  ...
  const deleteCategory = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        props.loader(true);
        Api("delete", `category/delete/${_id}`, data, router).then(
          (res) => {
      ...
      <section className="h-full w-full overflow-scroll no-scrollbar md:mt-0 mt-5 md:pb-32 pb-28">
        ...
        >
          ...
          <div className="flex justify-center items-center pt-5 pb-3">
            <button
              className="md:h-[40px] h-[40px] md:w-[274px] w-full bg-[var(--custom-blue)] rounded-[10px] md:text-lg text-base text-white "
              type="submit"
            >
              Add Now
            </button>
